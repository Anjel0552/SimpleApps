...
class UsersTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        userQuery?.findObjectsInBackgroundWithBlock({ [weak self] (objects, error) -> Void in
            ...
            guard let users = objects as? [PFUser] else { return }
            self?.users = users
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        // snder is the object that triggerd the segue
        // we are guarding to make sure it is all a UITableViewCell
        ...
        // use the indexPath.row to get the user for that cell
        // setting user on the convoVC to be the other person in the conversation
